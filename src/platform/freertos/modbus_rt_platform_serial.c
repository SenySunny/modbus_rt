...
static int modbus_rt_serial_add_dev(usart_type_dev * serial_port)
{
    int serial = 0;
...
static usart_type_dev *modbus_rt_serial_get_dev(int serial)
{
...
static void modbus_rt_serial_close_dev(int serial)
{
    ...
    usart_type_dev *serial_port = serial_port_temp->serial_port;
    os_uart_close(serial_port);
...
int modbus_rt_serial_open(const char *devname, int baudrate, int bytesize, char parity, int stopbits, int xonxoff) {
    usart_type_dev *serial_port = os_uart_open(devname, baudrate, bytesize, parity, stopbits,xonxoff);
    if(NULL == serial_port) {
...
void modbus_rt_serial_send(int serial, void *buf, int len) {
    usart_type_dev *serial_port = modbus_rt_serial_get_dev(serial);
    os_uart_send(serial_port, buf, len);
}

int modbus_rt_serial_receive(int serial, void *buf, int bufsz, const int timeout, const int bytes_timeout) {
    usart_type_dev *serial_port = modbus_rt_serial_get_dev(serial);
    return os_uart_receive(serial_port, buf, bufsz, timeout, bytes_timeout);
}
