...
int modbus_rt_net_segment(char *ipaddr, uint32_t saddr){
    if(NULL != ipaddr) {
        ...
        mask = 0x00FFFFFF;
#if  _ESP_NETIF_ENABLE
        esp_netif_t* netif_first = NULL;
        if(esp_netif_get_nr_of_ifs() > 0) {
            netif_first = esp_netif_next(NULL);
        }
        if(NULL != netif_first) {
            esp_netif_ip_info_t ip_info;
            if (esp_netif_get_ip_info(netif_first, &ip_info) == ESP_OK) {
                ip =  ip_info.ip.addr;
                // in_addr_t mask = ip_info.netmask.addr;
            }
        }
#else      
