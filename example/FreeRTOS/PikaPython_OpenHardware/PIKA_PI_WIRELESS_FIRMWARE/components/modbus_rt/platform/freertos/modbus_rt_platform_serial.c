...
static int modbus_rt_serial_add_dev(uart_type_dev * serial_port)
{
    int serial = 0;
...
static uart_type_dev *modbus_rt_serial_get_dev(int serial)
{
...
static void modbus_rt_serial_close_dev(int serial)
{
    ...
    uart_type_dev *serial_port = serial_port_temp->serial_port;
...
int modbus_rt_serial_open(const char *devname, int baudrate, int bytesize, char parity, int stopbits, int xonxoff) {
    uart_type_dev *serial_port = os_uart_open(devname, baudrate, bytesize, parity, stopbits,xonxoff);
    if(NULL == serial_port) {
...
void modbus_rt_serial_send(int serial, void *buf, int len) {
    uart_type_dev *serial_port = modbus_rt_serial_get_dev(serial);
